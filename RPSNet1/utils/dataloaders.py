...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            im3 = cv2.imread(path)
            #im0 = im0[60:260,0:480]
            #im0 = im0[140:340, 0:640]
        ...
        if self.transforms:
            ...
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im = np.ascontiguousarray(im)  # contiguous
            im4 = letterbox(im3, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize
            im4 = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im4 = np.ascontiguousarray(im3)  # contiguous

        return path, im, im0, self.cap, s, im4
...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                img, ir, labels = mixup(img, ir, labels, *self.load_mosaic(random.randint(0, self.n - 1)))
    ...
    def load_ir(self, i):  # zjq
        ...
        if ir is None:  # not cached
            ...
            return self.irs[i]  # img ##, hw_original, hw_resized
