...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    """
    if 'Segment' not in module_type:
        if len(x)==1:
            batch, channels, height, width = x.shape  # batch, channels, height, width
        else:
            batch = 1
            channels, height, width = x[0].shape  # batch, channels, height, width
        #batch, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features.png"  # filename
            img = x[0].cpu().transpose(0,1).sum(1).detach().numpy()
            multiple=640/img.shape[1]
            upsample = nn.Upsample(scale_factor=multiple, mode='nearest')
            out_img = upsample(x)
            image0 = out_img[0].cpu().transpose(0, 1).sum(1).detach().numpy()
            img = image0[12:212,:]#修改特征图显示范围，删除填充区域
            plt.imsave(f,img)
            plt.close()
            LOGGER.info(f'Saveing {f}...')
            '''
...
def feature_visualization_all(x, module_type, stage,  save_dir=Path('runs/predict-seg/exp')):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
    ...
    if 'Detect' not in module_type:
        if len(x)==1:
            ...
            batch, channels, height, width = x[1].shape  # batch, channels, height, width
        #batch, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features.png"  # filename
            if not os.path.exists(f):  # 检查目录是否存在
                os.makedirs(f)  # 如果不存在则创建目录
            img = x[0].cpu().transpose(0, 1).sum(1).detach().numpy()
            plt.imsave(f, img)
