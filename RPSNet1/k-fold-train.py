...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = SegmentationModel(cfg, input_mode = opt.input_mode,ch=3, nc=nc, anchors=hyp.get('anchors'),sr=opt.super,factor=down_factor).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                if opt.super:
                    pred,output_sr = model(imgs,opt.input_mode)
                else:
                    pred = model(imgs,opt.input_mode)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device), masks=masks.to(device).float())
                if opt.super:
                    if opt.input_mode =='IR':
                        sr_loss = 0.1*torch.nn.L1Loss()(output_sr,imgs)
                    if opt.input_mode =='RGB':
                        sr_loss = 0.1*torch.nn.L1Loss()(output_sr,imgs)
                    else:
                        sr_loss = 0.1*(torch.nn.L1Loss()(output_sr[:,0:3,:,:,],imgs)+torch.nn.L1Loss()(output_sr[:,3:,:,:,],imgs[:,0:1,:,:,]))
                    loss +=sr_loss
                if RANK != -1:
...
# ROOT / 'models/segment/yolov5n-SEG-SPD-MF.yaml'
def parse_opt(known=False):
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='yolov5n-seg.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/SiC.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default='D:/algorithm/yolov5-v7.0-2/data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--super', default=False, action='store_true', help='super_resolution')
    ...
    parser.add_argument('--evolve', type=int, nargs='?', const=5, help='evolve hyperparameters for x generations')
