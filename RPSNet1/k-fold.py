...
def parse_opt():
    ...
    parser.add_argument('--data', default= 'D:/algorithm/datasets/SiC')  # 数据集路径
...
def main(opt):
    ...
    # 从YAML文件加载类名
    A=sorted(dataset_path.rglob('SiC.yaml'))
    classes_file = sorted(dataset_path.rglob('SiC.yaml'))[0]
...
def split_images_labels(args):
    ...
    label = image.parents[2] / 'labels' / 'train' /f'{image.stem}.txt'
    if label.exists():
...
if __name__ == "__main__":
    ...
    model = YOLO('yolovn-seg.pt', task='train')
    # 从文本文件中加载内容并存储到一个列表中
    ds_yamls = []
