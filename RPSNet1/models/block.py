...
class MIRBModule(nn.Module):
    def __init__(self, in_channels, out_channels, shortcut=True, distillation_rate=0.25):
        super(MIRBModule, self).__init__()
        self.distilled_channels = int(in_channels * distillation_rate)
        self.remaining_channels = int(in_channels - self.distilled_channels)
        self.mid_channels = int(in_channels*distillation_rate*2)
        self.c1_1 = conv_layer(in_channels, self.distilled_channels, 1)
        self.c1_2 = conv_layer(in_channels, self.distilled_channels, 1)
        self.c1_3 = conv_layer(in_channels, self.distilled_channels, 1)
        self.c1_4 = conv_layer(in_channels, self.distilled_channels, 1)
        self.c2 = conv_layer(self.distilled_channels, self.distilled_channels, 3)
        self.c3 = conv_layer(self.mid_channels, self.mid_channels, 3)
        self.c4 = conv_layer(self.remaining_channels, self.remaining_channels, 3)
        self.act = activation('lrelu', neg_slope=0.05)
        self.c5 = conv_layer(self.distilled_channels * 4, out_channels, 1)
        self.cca = CCALayer(self.distilled_channels * 4)
        self.add = shortcut and in_channels == out_channels

    def forward(self, input):
        out_c1_1 = self.act(self.c1_1(input))
        out_c1_2 = self.act(self.c1_2(input))
        out_c1_3 = self.act(self.c1_3(input))
        out_c1_4 = self.act(self.c1_4(input))
        out_c2 = self.act(self.c2(out_c1_1))
        concat_1 = torch.cat((out_c1_2,out_c2),1)
        out_c3 = self.act(self.c3(concat_1))
        concat_2 = torch.cat((out_c1_3, out_c3), 1)
        out_c4 = self.act(self.c4(concat_2))
        concat_3 = torch.cat((out_c1_4,out_c4), dim=1)
        out_fused = self.c5(self.cca(concat_3))
        return out_fused + input if self.add else out_fused
