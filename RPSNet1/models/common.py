...
#from models.block import MIRBModule
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        #self.cv3 = nn.Conv2d(c_, c2, 3, 1, autopad(1, None, 1), groups=1, dilation=1, bias=False)
        #self.cv3 = nn.Conv2d(c_, c2, kernel_size=1, stride=1, bias=False)
        #self.sigmoid = torch.nn.Sigmoid()
        self.add = shortcut and c1 == c2

    def forward(self, x):
        #sim_att = self.sigmoid(self.cv3(self.cv1(x)))-0.5
        #out1 = x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class CCA(nn.Module):
    def __init__(self, channel1, reduction=16):
...
class C3_CCA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.CCA = CCA(c_, reduction=16)

    def forward(self, x):
        return self.cv3((torch.cat((self.CCA(self.m(self.cv1(x))), self.cv2(x)), 1)))

class C3_CCA1(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.CCA = CCA(2 * c_, reduction=16)

    def forward(self, x):
        #return self.cv3(self.CCA(torch.cat((self.m(self.cv1(x))), self.cv2(x), 1)))
        return self.cv3(self.CCA(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))
...
class C3_SPAB(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.SPAB = SPAB(2 * c_, bias=True)

    def forward(self, x):
        output = torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)
        output_SPAB = self.SPAB(output)
        return self.cv3(output_SPAB)
...
class C3_S(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(3 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.cv4 = nn.Conv2d(c_, c_, kernel_size=1, stride=1, bias=False)
        self.sigmoid = torch.nn.Sigmoid()

    def forward(self, x):
        sim_att = self.sigmoid(self.cv4(self.m(self.cv1(x))))-0.5
        sim_att_out = self.m(self.cv1(x)) * sim_att
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x),sim_att_out), 1))
'''
class C3_CCA(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c2, c2, 1)  # optional act=FReLU(c2)
        #self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.m = nn.Sequential(*(MIRBModule(c_, c_, shortcut) for _ in range(n)))
        #self.CCA = CCA(2 * c_, c2,reduction=16)
...
class C3(nn.Module):
    ...
    def forward(self, x):
        #sim_att = self.sigmoid(self.cv4(self.cv1(x)))-0.5
        #out = self.m(self.cv1(x)) * sim_att
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
