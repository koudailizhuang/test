...
from models.yolov5_object_segment import YOLOV5TorchObjectDetector
import cv2
...
target_layers = ['model-21-cv3-act','model-24-cv3-act','model-27-cv3-act']

# Arguments
parser = argparse.ArgumentParser()
parser.add_argument('--model-path', type=str, default=ROOT / 'runs/train-seg/exp99/weights/best.pt', help='Path to the model')
parser.add_argument('--img-path', type=str, default=ROOT / 'data/640_200', help='input image path')
parser.add_argument('--output-dir', type=str, default=ROOT / 'runs/gradcam_map/exp99_', help='output dir')
parser.add_argument('--img-size', type=int, default=640, help="input image size")
parser.add_argument('--target-layer', type=str, default='model-17-cv3-act',
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        #cv2.putText(img, label, (c1[0], c1[1] - 2 if outside else c2[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf,lineType=cv2.LINE_AA)
    return img
...
def main(img_path):
    ...
    for target_layer in target_layers:
        ...
        merged_mask = stacked_masks.mean(dim=0)
        res = result.copy()
        out_img, map = get_res_img(merged_mask, res)
        # 遍历每张图片中的每个目标
        for i, mask in enumerate(masks):
            ...
            out_img = plot_one_box(bbox, out_img, label=None, color=colors[int(names.index(cls_name))],line_thickness=3)
            # 缩放到原图片大小
        ...
        # 缩放到原图片大小
        out_img = cv2.resize(out_img, dsize=(img.shape[:-1][::-1]))
        out_path = f'{save_path}/{target_layer[6:8]}.jpg'
