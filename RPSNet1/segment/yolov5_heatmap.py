...
class yolov5_heatmap:
    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            #cam_image = self.draw_detections(post_boxes[i], self.colors[0], f'{self.model_names[0]} {post_result[i][0]:.2f}', cam_image)
            cam_image = Image.fromarray(cam_image)
            cam_image.save(f'{save_path}/{i}.png')

def get_params():
    params = {
        'weight': 'D:/algorithm/yolov5-v7.0-2/weight/best.pt',
        'cfg': 'D:/algorithm/yolov5-v7.0-2/models/segment/yolov5n-seg.yaml',
        'device': 'cuda:0',
        'method': 'XGradCAM', # GradCAMPlusPlus, GradCAM, XGradCAM
        'layer': 'model.model[-2]',
        'backward_type': 'class', # class or conf
        'conf_threshold': 0.25, # 0.6
        'ratio': 0.02 # 0.02-0.1
    }
    return params
...
if __name__ == '__main__':
    ...
    model('D:/algorithm/yolov5-v7.0-2/segment/1.JPG', 'D:/algorithm/yolov5-v7.0-2/segment/results')
